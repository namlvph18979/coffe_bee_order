...
class CartBottomBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return modelInvoice.listSp!.isNotEmpty
        ? BlocBuilder<DetailInvoiceBloc, CubitState>(
            bloc: invoiceBloc,
            builder: (context, state) {
              int count = 0;
              int priceAll = 0;
              for (int i = 0; i < modelInvoice.listSp!.length; i++) {
                if (modelInvoice.listSp![i].discountPercent != null) {
                  priceAll +=
                      ((modelInvoice.listSp![i].price * modelInvoice.listSp![i].soluong) *
                          (100 - modelInvoice.listSp![i].discountPercent!) ~/100);
                } else {
                  priceAll +=
                      (modelInvoice.listSp![i].price * modelInvoice.listSp![i].soluong);
                }
                count += modelInvoice.listSp![i].soluong;
              }
              return BottomSheet(
                  onClosing: () {},
